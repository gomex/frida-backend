...
function createHome(callback) {
  async.parallel({
    ...
    tabloid_ce: (callback) => createTabloid('tabloid_ce', callback),
    tabloid_mg: (callback) => createTabloid('tabloid_mg', callback),
    tabloid_pr: (callback) => createTabloid('tabloid_pr', callback),
    tabloid_pe: (callback) => createTabloid('tabloid_pe', callback),
    tabloid_rj: (callback) => createTabloid('tabloid_rj', callback)
  }, (err, results) => {
    if (err) return callback(err);
...
function  createTabloidNews(callback) {
  async.times(20, (n, callback) => {
    async.parallel([
      async.apply(createTabloidNew, 'pernambuco'),
      async.apply(createTabloidNew, 'parana'),
      async.apply(createTabloidNew, 'rio_de_janeiro'),
      async.apply(createTabloidNew, 'minas_gerais'),
    ], callback);
  }, callback);  
}
...
function createTabloidNew(regionalArea, callback) {
  var metadata = tabloidNewsMetadataFactory.build();
  var tabloidNew = new News(tabloidNewsFactory.build({regional_area: regionalArea}));
