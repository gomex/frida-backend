...
var tabloids = require('../news/tabloids');
...
function updateStatusOnDatabase(news, callback) {
  var updatedItem = _.clone(news);
  var newsId = updatedItem._id;
  delete updatedItem._id;

  News.findOneAndUpdate({_id: ObjectId(newsId)}, {$set: updatedItem}, {new: true}, callback);
}
...
function updateNews(news, callback) {
  news.save(callback);
}
...
var publish = function(news, callback) {
  ...
  if((news.status === 'draft') || newsHasChanged(news)) {
    ...
    pipeline.push(async.apply(updateNews, news));
...
var unpublish = function(news, callback) {
  ...
  pipeline.push(async.apply(updateStatusOnDatabase, news));
