...
exports.startServer = function() {
  ...
  router.put('/organization/:organization/:repository/posts/:id', function(req, res) {
    postsRepository.updateById(req.params.id, req.body, function(result) {
      res.statusCode = 200;
      res.json({ id : result });
      res.end();
    });
  ...
  router.put('/organization/:organization/:repository/posts/:id/status', function(req, res) {
    postsRepository.findById(req.params.id, function(post){
      publish.toYAML(post, post.metadata.title, '2015', '08').write(process.env.HEXO_POSTS_PATH, function(err, path){
