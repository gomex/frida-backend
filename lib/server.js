  ...
  router.put('/organization/:organization/:repository/posts/:id/status/:status', simpleAuth(EDITOR), function(req, res) {
    postsRepository.findById(req.params.id, function(post){
      ...
      var updateHome = function(layout) {
          ...
          postsRepository.getAll(function(allNews){
            var featured    = allNews.splice(FEATURED_LIMIT);
            var secondary   = allNews.splice(SECONDARY_LIMIT);
            var tertiary    = allNews.splice(TERTIARY_LIMIT);

            var homePosts = {featured:featured, secondary:secondary, tertiary:tertiary};
