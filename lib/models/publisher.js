...
function publisherWorker(tasks, callback) {
  var tasksByKind = _.groupBy(tasks, 'name');
  var newsToPublish = _.pluck(tasksByKind['publish'], 'id');
  var newsToUnpublish = _.pluck(tasksByKind['unpublish'], 'news');

  hexoSource.removePosts((err) => {
    if(err) return callback(err);

    async.parallel([
      async.apply(publishNew, newsToPublish),
      async.apply(unpublishNew, newsToUnpublish)
    ], (err) => {
      if(err) return callback(err);
      ...
      async.parallel([
        ...
        hexo.generate('publish', () => { callback(err); });
      });
    });
  });
}
...
function publishLater(news, priority, callback) {
  ...
  var ids = _.map(news, (news) => news._id);
  News.updateMany({ _id: { $in: ids }}, { $set: { status: 'publishing' } }, (err) => {
    if (err) return callback(err);

    var publications = _.map(ids, (id) => ({ name: 'publish', id: id }));
    var add = priority ? publisherCargo.unshift : publisherCargo.push;
    add(publications, (err) => {
...
function unpublishLater(news, callback) {
  ...
  news.save((err, news) => {
    ...
    publisherCargo.unshift({ name: 'unpublish', news: news }, (err) => {
...
function ifHasTabloidPublish(news, callback) {
  ...
  tabloids.findTabloid(news, (err, tabloid) => {
    ...
    if(process.env.TOGGLE_qVIq5Tnp_INCREMENTAL_GEN == 'enabled') {
      publishNew([tabloid._id], (err) => callback(err, news));
    } else {
      publishOld([tabloid], (err) => callback(err, news));
    }
  });
}
...
function publishNew(news, callback) {
  ...
  News.find().where('_id').in(news).exec((err, results) => {
    async.mapSeries(results, publishOne, callback);
  });
}
...
function unpublishOne(news, callback) {
  ...
  var removeStrategy = null;
  if(process.env.TOGGLE_qVIq5Tnp_INCREMENTAL_GEN == 'enabled') {
    removeStrategy = async.apply(site.remove, news.metadata.url);
  } else {
    removeStrategy = async.apply(hexo.unpublish, news);
  }
