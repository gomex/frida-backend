...
function publisherWorker(tasks, callback) {
  ...
  hexoSource.removePosts((err) => {
    ...
    async.parallel([
      async.apply(enrichesAndPublish, newsToPublish),
...
function enrichesAndPublish(newsIds, callback) {
  News.find().where('_id').in(newsIds).exec((err, results) => {
    if (err) return callback(err);

    publishNew(results, callback);
  });
}
