...
function publishNewsArea(news, callback) {
  if (!news.isPost()
    && !news.isTabloidNews()
    && !news.isSpecial()) {
  ...
  News.find().publisheds().byArea(news.metadata.area)
...
function publishColumns(news, callback) {
  if (!news.isColumn()) {
    return callback();
  }
...
function publishColumnistAreaDataFile(news) {
  ...
  if (news.isColumn()) {
    columnist = news.metadata.columnist;
    return async.apply(hexo.updateColumnistPage, columnist);
  }
...
var publish = function(news, callback) {
  ...
  var pipeline = [
    ...
    async.apply(publishNewsArea, news),
    async.apply(publishColumns, news),
  ...
  async.series(pipeline, function(err) {
    if(process.env.TOGGLE_fKc6ol5N_GENERATE_ON_PUBLISH == 'enabled') {
