...
function publisherWorker(tasks, callback) {
  var tasksByKind = _.groupBy(tasks, (task) => { task.name; });
  var newsToPublish = _.pluck(tasksByKind['publish'], 'news');
  ...
  hexoSource.removePosts((err) => {
    ...
    async.parallel([
      async.apply(publish, newsToPublish),
      async.apply(unpublish, newsToUnpublish)
    ], (err) => {
      if(err) return callback(err);
...
function publishLater(news, callback) {
  news.status = 'publishing';

  news.save((err, news) => {
    if (err) return callback(err);

    publisherCargo.unshift({ name: 'publish', news: news }, (err) => {
...
function publish(news, callback) {
  if(process.env.TOGGLE_qVIq5Tnp_INCREMENTAL_GEN == 'enabled') {
    return publishNew(news, callback);
  } else {
    return publishOld(news, callback);
  }
}
...
function unpublishNew(news, callback) {
  async.mapSeries(news, unpublishOne, callback);
}

function unpublish(news, callback) {
  if(process.env.TOGGLE_qVIq5Tnp_INCREMENTAL_GEN == 'enabled') {
    return unpublishNew(news, callback);
  } else {
    return unpublishOld(news, callback);
  }
}
