...
function publishAreaList(news, callback) {
  if ((!news.isPost()
    && !news.isTabloidNews()
    && !news.isSpecial())
  ...
  News.find().publisheds().byArea(news.metadata.area)
    ...
    .exec((err, list) => {
      ...
      hexo.publishList({
        layout: 'news_list',
        path: News.getAreaListPath(news.metadata.area),
...
function publishLastNewsList(news, callback) {
  ...
  News.find()
    .publisheds()
    .byLayouts(['column', 'tabloid_news', 'column', 'special'])
    .sort('-published_at')
    .limit(40)
    .exec((err, list) => {
  ...
  News.find().publisheds().byColumnist(news.metadata.columnist)
    ...
    .limit(40).exec((err, list) => {
      ...
      hexo.publishList({
        ...
        area: columnists.getColumnistListPath(news.metadata.columnist),
        news: list
      }, callback);
    });
}
...
var publish = function(news, callback) {
  ...
  var pipeline = [
    ...
    async.apply(publishAreaList, news),
