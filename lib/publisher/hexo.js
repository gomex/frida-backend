...
var postStrategy    =  require('./post-strategy');
...
function publishWithOtherNews(news, otherNewsData, callback) {
  var filedir = getSourceDir(news);
  async.series([
  ...
  publish: function(news, callback){
    postStrategy.buildOtherNewsData(news, function(err, otherNewsData){
      if(err) {
        ...
        publishWithOtherNews(news, otherNewsData, callback);
      }
    });
  },
