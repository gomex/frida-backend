...
function addSuffixToTrickHexo(date) {
  return date + '####';
}

function addSuffixToDateField(item) {
  if(_.has(item, 'date')) {
    item.date = addSuffixToTrickHexo(item.date);
  }
  return item;
}
...
function createSiteIndexDataFile(homeNews, callback) {
  mkdirp(HEXO_SOURCE_PATH, function(err){
    if(err) {
      ...
      homeNews = _.mapObject(homeNews, addSuffixToDateField);
      homeNews.last_news = _.map(homeNews.last_news, addSuffixToDateField);
      homeNews.most_read = _.map(homeNews.most_read, addSuffixToDateField);
...
function createAreaIndexDataFile(areaName, areaPageData, callback) {
  ...
  mkdirp(areaIndexDir, function(err) {
    if(err) {
      ...
      areaPageData = _.map(areaPageData.news, addSuffixToDateField);
...
function createNewsDataFile(news, callback) {
  ...
  newsMetadataClone.date = addSuffixToTrickHexo(news.published_at);
  if (news.issuu)
