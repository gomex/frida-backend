...
var HEXO_DEST_PATH    = process.env.HEXO_DEST_PATH;
...
function getDestPath(news) {
  return path.join(HEXO_DEST_PATH, news.metadata.url, 'index.html');
}
...
function unpublish(news, callback) {
  var removeFile = (filepath, callback) => {
  ...
  async.series([
    async.apply(removeFile, getSourcePath(news)),
    async.apply(removeFile, getDestPath(news))
  ], callback);
}
