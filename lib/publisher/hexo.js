...
function publishStaticFiles(callback) {
  async.series([
    async.apply(staticFiles.generate, 'quem_somos', 'static_about'),
    async.apply(staticFiles.generate, 'contato', 'static_contact'),
    async.apply(staticFiles.generate, 'mapa_do_site', 'static_sitemap'),
    async.apply(staticFiles.generate, 'publicidade', 'static_advertising'),
    async.apply(staticFiles.generate, 'parceiros', 'partners')
  ], callback);
}
