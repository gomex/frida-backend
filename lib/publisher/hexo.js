...
var cargoOfPostsToPublish = async.cargo((pendingPublicationPosts, callback) => {
  console.log('Site generation started.');
  execFile(path.join(process.env.HEXO_SITE_PATH, 'hexo-deployer.sh'),
      [process.env.HEXO_SITE_PATH, process.env.HEXO_SITE_DEST, 'dont_clean'],
      (error, stdout, stderr) => {
        console.log('stdout:\n', stdout);
...
function generate(news, callback) {
  cargoOfPostsToPublish.push(news, callback);
}
