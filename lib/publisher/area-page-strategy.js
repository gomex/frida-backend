...
function createColumnistFrontMatter(columnist, result, numPages) {
  return function(i) {
    return {
      layout: 'columnist_list',
      news: result.slice(i*NEWS_LIMIT, (i+1)*NEWS_LIMIT),
      columnist: Utils.toDisplayName(columnist),
      current: i,
      total: numPages,
      next: (i < numPages-1),
      prev: (i > 0)
...
function buildColumnistPageData(columnist, callback) {
  var pageData = [];

  News.findNews(Utils.publishedColumnsFor(columnist), Utils.newsProjection, NEWS_LIMIT*NEWS_PAGES, Utils.sortByPublishedDate, function(err, result) {
    if(!err) {
      var numPages = Math.ceil(result.length/NEWS_LIMIT);
      if(numPages > 0) {
        pageData = _.range(numPages).map(createColumnistFrontMatter(columnist, result, numPages));
      }
      else {
        pageData = [{
          layout: 'columnist_list',
