...
function buildAreaPageData(area, callback) {
  ...
  pipeline.push({ '$match': publishedNewsFor(area) });
  pipeline.push({ '$sort': sortByPublishedDate });
  pipeline.push({ '$limit': NEWS_LIMIT });
  pipeline.push({ '$project': newsProjection });

  News.aggregate(pipeline, function(err, result) {
    if(!err) {
...
function buildColumnsPageData(callback) {
  ...
  pipeline.push({ '$match': publishedColumns });
  pipeline.push({ '$sort': sortByPublishedDate });
  pipeline.push({ '$limit': NEWS_LIMIT });
  pipeline.push({ '$project': columnProjection });
