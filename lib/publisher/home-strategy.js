...
var mostReadCriteria = {
  'status': 'published',
  'metadata.layout': 'post',
  'most_read': true
};

var photoCaptionCriteria = {
  'status': 'published',
...
function buildHome(callback) {
  ...
  async.parallel({
    ...
    most_read: async.apply(newsRepository.find, mostReadCriteria, newsProjection, 5, sortByPublishedDate),
    photo_caption: async.apply(newsRepository.find, photoCaptionCriteria, photoCaptionProjection, 1, sortByPublishedDate)
  }, function(err, result){
    if(!err) {
