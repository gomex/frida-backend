...
function buildHome(callback) {
  ...
  async.parallel([
    ...
    async.apply(newsRepository.find, publishedColumnCriteria('column_01'), columnProjection, 1, sortByPublishedDate),
    async.apply(newsRepository.find, publishedColumnCriteria('column_02'), columnProjection, 1, sortByPublishedDate),
    async.apply(newsRepository.find, publishedColumnCriteria('column_03'), columnProjection, 1, sortByPublishedDate),
    async.apply(newsRepository.find, lastNewsCriteria, newsProjection, 6, sortByPublishedDate)
  ], function(err, news){
    news = _.flatten(news);
    ...
    newsForHome.column_01   = news[8];
    newsForHome.column_02   = news[9];
    newsForHome.column_03   = news[10];
    newsForHome.last_news   = news.slice(-6);
